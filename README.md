...
Where the grid cannot show something honestly it is named underneath rather than improvised. A passage in or out has nowhere to point on a flat map. A staircase gets a dotted line rather than the solid one a passage north gets, because the room above it was put north only to be readable and is not really north of anywhere. And a passage whose two rooms have drifted out of line, which happens as the map is shoved aside to make space elsewhere, is listed instead of drawn as a line that would have to bend around whatever now stands between them.

Two things limit what can be mapped. Only a game that says where the player is can be: the Z-Machine keeps the room in a global variable through Version 3, because the interpreter draws the status line and has to know what to write on it, and from Version 4 a game draws its own and may keep the room anywhere, so nothing is drawn for those. And a line holding several commands, which these parsers allow and which walkthroughs are full of, names no direction at all, because the parser walks every room on it and only the one the player finished in is ever seen; the same walkthrough written one command to a line maps far better than one written `n. n. u`.
