...
What it cannot do is make these maps flat. Zork's geometry is very nearly consistent, with only sixteen of its hundred and forty-six passages contradicting the arithmetic outright, and yet half its rooms want a cell another room has already taken: the map folds over itself, with the underground lying across the ground above it. Splitting a map into separate sheets is what answers that, and it is not built yet, so about a quarter of a large game's passages are still named underneath rather than drawn.
