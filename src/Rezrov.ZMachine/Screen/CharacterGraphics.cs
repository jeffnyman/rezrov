...
public static class CharacterGraphics
{
    ...
    /// <summary>
    /// [zm 16.1] The font itself: one line of sixty-four cells per
    /// character, the eight rows of eight laid end to end, a hash for
    /// a cell that is on and a period for one that is off.
    /// </summary>
    /// <remarks>
    /// This is the font rather than an approximation of it, and a
    /// frontend that owns its pixels draws these instead of asking
    /// <see cref="ToUnicode"/> for the nearest thing a text font has.
    /// The cells were read out of the standard's own listing rather
    /// than typed in by eye, and a test reads that listing again and
    /// compares every glyph against this table, so the two cannot
    /// drift apart and a misreading cannot survive.
    /// </remarks>
    ...
    // The same table packed a row to a byte, since that is the shape
    // anything drawing pixels wants it in. [zm 16.1] The standard
    // numbers the cells of a row from seven down to zero, left to
    // right, so the leftmost cell is the top bit.
    private static readonly byte[] Rows = Pack(Bitmaps);

    private static readonly byte[] Blank = new byte[Height];

    /// <summary>The cells across one character of font 3.</summary>
    public const int Width = 8;

    /// <summary>The rows down one character of font 3.</summary>
    public const int Height = 8;

    /// <summary>
    /// [zm 16.1] The eight rows of a font 3 character, each row a byte
    /// whose top bit is its leftmost cell. A code outside 32 to 126 has
    /// no glyph in the font and comes back blank.
    /// </summary>
    public static ReadOnlySpan<byte> Bitmap(char character) =>
        character is >= ' ' and <= '~'
            ? Rows.AsSpan((character - ' ') * Height, Height)
            : Blank;

    private static byte[] Pack(string[] bitmaps)
    {
        var packed = new byte[bitmaps.Length * Height];

        for (var glyph = 0; glyph < bitmaps.Length; glyph++)
        {
            for (var row = 0; row < Height; row++)
            {
                ...
                {
                    if (bitmaps[glyph][(row * Width) + cell] == '#')
                    {
                        bits |= 1 << (Width - 1 - cell);
                    }
                }

                packed[(glyph * Height) + row] = (byte)bits;
            }
        }
