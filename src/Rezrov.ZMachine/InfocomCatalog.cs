...
/// <summary>
/// [babel legacy Z-code IFID] Infocom's own catalog: the games that
/// came before there was any way for a story to say what it was.
/// </summary>
/// <remarks>
/// Infocom shipped two decades before the Treaty of Babel, and their
/// compiler left the header bytes that a later one writes its name
/// into empty. So did early Inform, which means an empty compiler
/// field says nothing: Curses looks exactly like Trinity there. The
/// two machines Dialog and Arcturus compile to leave it empty as well.
///
/// The catalog is finite and closed, though, so the identities are
/// simply known. Every identifier here is one the treaty's rules work
/// out from the three numbers in a story's header, and the titles are
/// those of Andrew Plotkin's Obsessively Complete Infocom Catalog
/// (https://eblong.com/infocom/, its catalog.json, read on
/// 2026-08-23).
///
/// Two of the catalog's entries are worth knowing about. Release 5
/// with a serial of nothing at all is left out: Suspended and Zork I
/// both shipped under it, so that identity is genuinely ambiguous and
/// naming it either way would be a guess. And the Leather Goddesses
/// beta is named with its checksum as well, because its serial is not
/// a date and the treaty gives such a file a checksum to tell it from
/// its siblings.
/// </remarks>
public static class InfocomCatalog
{
    /// <summary>
    /// The title of the game a story file is, or null where it is not
    /// one of Infocom's.
    /// </summary>
    public static string? Title(string? ifid) =>
        ifid is not null && Titles.TryGetValue(ifid, out var title) ? title : null;

    /// <summary>Whether a story file is one of Infocom's.</summary>
    public static bool Knows(string? ifid) => Title(ifid) is not null;

    /// <summary>
    /// The title of a story file itself, or null where it is not one
    /// of Infocom's. A frontend with a window to name asks this and
    /// falls back to the name of the file.
    /// </summary>
    public static string? TitleOf(ReadOnlySpan<byte> story) => Title(Ifid.Of(story));

    /// <summary>How many identities the catalog names.</summary>
    public static int Count => Titles.Count;
